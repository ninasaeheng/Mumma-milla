...
function handleBundleVariantChange(event) {
  ...
  if (currentVariant) {
    const productFormUpdate = productForm.closest('.pb_bottom_detail');
    if (productFormUpdate) {
      productFormUpdate.closest('.pb__sibar').querySelector('product-form input[name="id"]').value = currentVariant.id;
    }
