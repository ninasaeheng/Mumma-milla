...
// product image gallery script starts here 
 ...
 $('.custom-slider-for').slick({
  ...
  arrows: true,
  fade: true,
  prevArrow: '<div class="slick-slider__prev"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="1.5" stroke-linecap="square" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg></div>',
  nextArrow: '<div class="slick-slider__next"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="1.5" stroke-linecap="square" stroke-linejoin="round"><path d="M9 18l6-6-6-6"/></svg></div>',
...
$slickElement.on('afterChange', function (event, slick, currentSlide, nextSlide) {
  ...
  $('.pb_top_detail [data-id="'+element.getAttribute('data-id')+'"]').removeClass("hidden").addClass("selected").siblings().addClass("hidden").removeClass("selected");
  totalPrice();
});
$slickElementBottom.on('afterChange', function (event, slick, currentSlide, nextSlide) {
  var current_slider= slick.$slides.get(currentSlide);
  var element = current_slider.querySelector('.pb__product');
  $('.pb_bottom_detail [data-id="'+element.getAttribute('data-id')+'"]').removeClass("hidden").addClass("selected").siblings().addClass("hidden").removeClass("selected");
  totalPrice();
});
...
function totalPrice(){
  ...
  $('.total_price .tp_ptice').text(Shopify.formatMoney(total_price,cartStrings?.money_format));
}
...
console.log(product_form_update);
          product_form_update.closest('.pb__sibar').find('product-form input[name="id"]').val(current_variant.id);
        }
        product_form.attr('v-id',current_variant.id);
        product_form.find('.pd_price').attr('data-price',current_variant.price);
        product_form.find('.pd_price .price').text(Shopify.formatMoney(current_variant.price,cartStrings?.money_format));
        totalPrice();
    }
// console.log(current_variant);
});
...
$(document).ready(function(){
    ...
    $('.pb_bottom_products .product__slider').on('afterChange', function(event, slick, currentSlide){
